...
mkdir -p bin/opt/vlfeat-0.9.21
mkdir -p bin/vlfeat
rm -rf bin/vlfeat
rm -rf bin/opt/vlfeat-0.9.21
cd bin/opt/download
wget -N http://www.vlfeat.org/download/vlfeat-0.9.21-bin.tar.gz || exit 1
...
if [[ $sum = *$vlsum* ]]
  then
    ...
    tar -xzvf vlfeat-0.9.21-bin.tar.gz || exit 1
    cp vlfeat-0.9.21/bin/glnxa64/libvl.so ../../
    mv vlfeat-0.9.21 ../ || exit 1
    echo "done"
  else
